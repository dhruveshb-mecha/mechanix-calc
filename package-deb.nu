...
def main [
    ...
    let control_lines = if ($dependencies | is-empty) {
        $control_lines
    } else {
        ($control_lines | append $"Depends: ($dependencies)")
    }
    let control_content = (($control_lines | append $"Description: ($app_description)") | str join "\n") + "\n"
    $control_content | save -f $"($pkg_dir)/DEBIAN/control"

    let binary_src = ($bundle_dir | path join $app.binary)
    let bundle_dest = $"($pkg_dir)/usr/share/mechanix/($pkg_name)"
    let binary_dest = $"($bundle_dest)/($app.binary)"
